...
- `/pet` が表示されている間だけ companion helper を起動し、`/pet` が閉じると helper を停止します。
- Windows tray icon は既定でオフなので、Codex アプリがもう 1 つあるようには見えません。
...
Installer はファイルを `%LOCALAPPDATA%\CodexyPetUsagesRing` にコピーし、
軽量な `/pet` watcher を開始して Windows スタートアップに登録します。
...
インストール済みの場合、launcher は `%LOCALAPPDATA%\CodexyPetUsagesRing`
...
リング/バッテリー/バッジ表示を選び、色、不透明度、readout 色、hover text
