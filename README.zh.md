...
- 在当前 Codex `/pet` 头像周围显示圆环、小型电池条或徽章标签。
- 添加可选 pet 成长：今日 XP 会按 5h 使用量进度填充，并保留每周重置赛季和更有趣的状态名称。
- 添加实时键盘计数、低概率奖励掉落、可点击奖励箱，以及字体/主题外观解锁。
- 悬停时显示 5h 限制和每周限制的用量信息。
- 将 readout、tray text 和设置 UI 本地化为英语、韩语、日语和中文。
- 自动检测 Codex Desktop，并可在需要时启动它。
- 仅在 `/pet` 可见时启动 companion helper，并在 `/pet` 关闭后停止 helper。
- Windows tray icon 默认关闭，因此不会看起来像多开了一个 Codex 应用。
...
Installer 会将文件复制到 `%LOCALAPPDATA%\CodexyPetUsagesRing`，启动轻量级
`/pet` watcher，并将其注册到 Windows 启动项。
...
你可以在圆环/电池/徽章显示之间切换，并更改颜色、透明度、readout 颜色和 hover
text 大小。pet 成长可以随时关闭，成长方式会选择今日 XP 对应的 5h 使用量目标:
轻量 20%、均衡 40%、专注 60%。每周使用量不再作为普通 XP 条件，只用于重置和防耗尽判断。

游戏化面板可控制键盘计数器和奖励显示模式。奖励掉落刻意设置为低概率；
只有已获得的外观会出现在箱子 inventory 中，并可应用为字体样式、主题键盘计数边框或 paw-burst typing effect。

正在运行的 helper 会自动重新加载设置文件。
